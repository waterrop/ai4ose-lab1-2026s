            ...
            loop {
                ...
                let finish = match scause::read().cause() {
                    ...
                    Trap::Exception(Exception::UserEnvCall) => {
                        ...
                        // 设置当前 TCB 指针，供 trace 系统调用使用
                        unsafe { task::set_current_tcb(tcb) };
                        let event = tcb.handle_syscall();
                        // 清除当前 TCB 指针
                        unsafe { task::clear_current_tcb() };
                        match event {
...
mod impls {
    ...
    impl Trace for SyscallContext {
        ...
        fn trace(
            ...
            // 从模块级引用获取当前 TCB
            let tcb = match task::get_current_tcb() {
                Some(t) => t,
                None => return -1,
            };
            ...
            match trace_request {
                ...
                2 => {
                    // 查询系统调用次数
                    tcb.syscall_counts.get(id).copied().unwrap_or(0) as isize
                }
                _ => -1,
            }
        }
