...
/// 全局任务控制块数组
static TASKS: SyncUnsafeCell<[TaskControlBlock; APP_CAPACITY]> =
    SyncUnsafeCell::new([TaskControlBlock::ZERO; APP_CAPACITY]);
...
extern "C" fn rust_main() -> ! {
    ...
    let mut tcbs = unsafe { &mut *TASKS.get() };
    let mut index_mod = 0;
...
mod impls {
    ...
    impl Trace for SyscallContext {
        #[inline]
            ...
            match trace_request {
                // 读取用户内存：id 为 *const u8
                0 => {
                    let ptr = id as *const u8;
                    unsafe { *ptr as isize }
                }
                // 写入用户内存：id 为 *mut u8，data 为要写入的值
                1 => {
                ...
                2 => {
                    // 获取当前任务索引
                    let task_idx = crate::CURRENT_TASK.load(core::sync::atomic::Ordering::Relaxed);
                    // 获取任务数组
                    let tasks = unsafe { &*crate::TASKS.get() };
                    // 返回系统调用计数（注意：本次调用已在 handle_syscall 中计入）
                    if id < 512 {
                        tasks[task_idx].syscall_count[id] as isize
                    } else {
                        -1
                    }
                }
