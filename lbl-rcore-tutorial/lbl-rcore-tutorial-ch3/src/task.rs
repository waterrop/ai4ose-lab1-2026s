...
/// 模块级可变引用：指向当前正在处理的 TaskControlBlock
static mut CURRENT_TCB: Option<&'static mut TaskControlBlock> = None;

/// 设置当前正在处理的 TCB
pub fn set_current_tcb(tcb: &'static mut TaskControlBlock) {
    unsafe { CURRENT_TCB = Some(tcb); }
}

/// 清除当前 TCB 引用
pub fn clear_current_tcb() {
    unsafe { CURRENT_TCB = None; }
}

/// 获取当前 TCB 的可变引用
pub fn get_current_tcb() -> Option<&'static mut TaskControlBlock> {
    unsafe { CURRENT_TCB.as_mut().map(|r| *r) }
}
